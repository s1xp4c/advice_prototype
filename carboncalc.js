  ...
  Promise.all([co2Promise, lighthousePromise])
    ...
    .catch((e) => {
      ...
      if (errorValue <= 5) {
        ...
        alert(
          "The carbon fetch request has failed!! - Please try again later or input another site"
        );
      }
      console.log(e);
    });
...
function fillInfo(co2Data, data) {
  ...
  // Unused CSS rules
  try {
    siteInfo.unusedCSStotalBytes =
      data.lighthouseResult.audits[
        "unused-css-rules"
      ].details.items[0].totalBytes.toPrecision(4) / divider;
  } catch (error) {
    siteInfo.unusedCSStotalBytes = zero;
  }
  try {
    siteInfo.unusedCSSwastedBytes =
      data.lighthouseResult.audits[
        "unused-css-rules"
      ].details.items[0].wastedBytes.toPrecision(4) / divider;
  } catch (error) {
  ...
  try {
    siteInfo.unusedCSSwastedPercent =
      data.lighthouseResult.audits[
        "unused-css-rules"
      ].details.items[0].wastedPercent.toPrecision(4);
  } catch (error) {
  ...
  // Responsive Images
  try {
    siteInfo.responsiveImagesTotalBytes =
      data.lighthouseResult.audits[
...
function displayInfoList(siteInfo) {
  ...
  document.querySelector("[data-field=unusedJStotalBytes]").textContent =
    "Total: " + siteInfo.unusedJStotalBytes + " kB";
  // unusedJSwastedBytes: "",
  document.querySelector("[data-field=unusedJSwastedBytes]").textContent =
    "Wasted: " + siteInfo.unusedJSwastedBytes + " kB";
  // unusedJSwastedPercent: "",
  ...
  document.querySelector("[data-field=unminCSStotalBytes]").textContent =
    "Total: " + siteInfo.unminCSStotalBytes + " kB";
  // unminCSSwastedBytes: "",
  document.querySelector("[data-field=unminCSSwastedBytes]").textContent =
    "Wasted: " + siteInfo.unminCSSwastedBytes + " kB";
  // unminCSSwastedPercent: "",
  ...
  document.querySelector("[data-field=unminJStotalBytes]").textContent =
    "Total: " + siteInfo.unminJStotalBytes + " kB";
  // unminJSwastedBytes: "",
  document.querySelector("[data-field=unminJSwastedBytes]").textContent =
    "Wasted: " + siteInfo.unminJSwastedBytes + " kB";
  // unminJSwastedPercent: "",
  ...
  ).textContent = "Total: " + siteInfo.modernImageFormatTotalBytes + " kB";
  // modernImageFormatWastedBytes: "",
  document.querySelector(
    "[data-field=modernImageFormatWastedBytes]"
  ).textContent = "Wasted: " + siteInfo.modernImageFormatWastedBytes + " kB";
  // modernImageFormatWastedPercent: "",
  document.querySelector(
  ...
  ).textContent = "Total: " + siteInfo.responsiveImagesTotalBytes + " kB";
  // responsiveImagesWastedBytes: "",
  document.querySelector(
    "[data-field=responsiveImagesWastedBytes]"
  ).textContent = "Wasted: " + siteInfo.responsiveImagesWastedBytes + " kB";
  // responsiveImagesWastedPercent: "",
  document.querySelector(
  ...
  document.querySelector("[data-field=optimizedImagesTotalBytes]").textContent =
    "Total: " + siteInfo.optimizedImagesTotalBytes + " kB";
  // optimizedImagesWastedBytes: "",
  document.querySelector(
    "[data-field=optimizedImagesWastedBytes]"
  ).textContent = "Wasted: " + siteInfo.optimizedImagesWastedBytes + " kB";
  // optimizedImagesWastedPercent: "",
  document.querySelector(
...
function changeInfoList(siteInfo) {
  ...
  // unusedCSStotalBytes: "",
  document.querySelector("[data-field=unusedCSStotalBytesChange]").textContent =
    "Total: " + siteInfo.unusedCSStotalBytes + " kB";
  // unusedCSSwastedBytes: "",
  document.querySelector(
    "[data-field=unusedCSSwastedBytesChange]"
  ).textContent = "Wasted: " + siteInfo.unusedCSSwastedBytes + " kB";
  // unusedCSSwastedPercent: "",
  document.querySelector(
  ...
  ).textContent = "Total: " + siteInfo.optimizedImagesTotalBytes + " kB";
  // optimizedImagesWastedBytes: "",
  document.querySelector(
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  slider.addEventListener("change", () => {
    console.log("Changed Input", siteInfoMirror);
    let siteInfo = siteInfoMirror;
    let sliderValue = slider.value;
    console.log("Number Change : ", sliderValue);
    // unusedCSStotalBytes: "",
    document.querySelector(
      "[data-field=unusedCSStotalBytesChange]"
    ).textContent = "Total: " + siteInfo.unusedCSStotalBytes + " kB";
    // unusedCSSwastedBytes: "",
    document.querySelector(
      "[data-field=unusedCSSwastedBytesChange]"
    ).textContent = "Wasted: " + siteInfo.unusedCSSwastedBytes + " kB";
    ...
    document.querySelector(
      "[data-field=unusedCSSwastedPercentChange]"
    ).textContent = "Wasted: " + siteInfo.unusedCSSwastedPercent + " %"; // unusedJStotalBytes: "",
    document.querySelector("[data-field=unusedJStotalBytesChange]").textContent =
      "Total: " + siteInfo.unusedJStotalBytes + " kB";
    // unusedJSwastedBytes: "",
    document.querySelector("[data-field=unusedJSwastedBytesChange]").textContent =
